...
app = FastAPI(title="EcoFit Carbon Coach MCP", version="2.0.0")
...
CO2_FACTORS = {
    ...
    "shopping": {"Groceries & Food": 1.0, "Clothing & Fashion": 2.5, "Electronics & Gadgets": 3.0, "Home & Living": 1.5, "Beauty & Personal Care": 1.2},
    "electronics_freq": {"Every year": 2.5, "Every 2-3 years": 1.0, "Rarely": 0.3}
}
UNIQUE_TIPS = {
    "transport": {"Car": ["Plan routes to avoid traffic jams — idling wastes fuel and CO₂!"], "Bus": ["Travel during off-peak hours to reduce congestion."],
                  "Bicycle": ["Keep your bike tires well-inflated."], "Walking": ["Walk to local shops to cut emissions."],
                  "Electric Scooter": ["Charge during off-peak hours."]},
    "shopping": {"Groceries & Food": ["Freeze leftovers to reduce food waste."], "Clothing & Fashion": ["Choose organic cotton or recycled fabrics."],
                 "Electronics & Gadgets": ["Buy refurbished devices."], "Home & Living": ["Use LED bulbs."],
                 "Beauty & Personal Care": ["Switch to biodegradable or refillable products."]},
    "electronics_freq": {"Every year": ["Try using devices for 2-3 years instead of replacing annually."],
...
FALLBACK_RESPONSES = ["Oops! Please try something else! 😊", "Hmm… Could you try another input? 🌱"]
...
async def carbon_score(request: Request):
    ...
    if mode == "calculate":
        ans = data.answers or AnswersModel(transport=data.transport, shopping=data.shopping, electronics_freq=data.electronics_freq)
        if not (ans.transport and ans.shopping and ans.electronics_freq):
            return {"message": "Please provide all answers."}
        score = sum([CO2_FACTORS["transport"][ans.transport], CO2_FACTORS["shopping"][ans.shopping], CO2_FACTORS["electronics_freq"][ans.electronics_freq]])
    ...
    if mode == "challenge":
        ...
        msg = "You are more eco-friendly! 🌟" if data.my_score < data.friend_score else "Your friend is more eco-conscious. 💪" if data.my_score > data.friend_score else "Same footprint. 🤝"
